...
from simple_planner import SimplifiedPlanner
...
def main(controller):
    # manuever specific
    MANUEVER_NAME = "0_freeflight"
    dataset_name = MANUEVER_NAME
    port = 25556
    nan = float('NaN')

    # exp specific
    SIM_MAX_DURATION = 9.0#33.0
    HOVER_DURATION = 3.0#22.0
    ...
    planner = SimplifiedPlanner(velocity=target_velocity, dt=0.005, hover_time=hover_time, 
                      start=(0.0,-2.5,-1.5), 
                      end=(0.0,15.0,-1.5), 
                      target_yaw=yaw_uav_2, 
                      #traj_type=0
                      )
    ...
    while curr_sim_time < SIM_MAX_DURATION:
        ...
        if False and curr_sim_time < HOVER_DURATION:
