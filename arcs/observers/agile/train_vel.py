...
import torch, sys
from model import AgileVelPredictor
from dataset import AgileVelDataset
...
def train():
    ...
    exp_name = init_experiment(f"Agile-Vel-half-data-sn-{str(sn_gamma)}-123")

    dataset = AgileVelDataset([
    ...
    if load_model:
        model = AgileVelPredictor().to(device)
