...
from dataset import NeuralSwarmDataset
import numpy as np
...
model_name = f"NeuralSwarm-sn-{str(sn_gamma)}-123S"
...
def train():
    ...
    exp_name = init_experiment(f"NSwarm-sn-{str(sn_gamma)}-123S")

    dataset = NeuralSwarmDataset([
    ...
    x_train, x_val, y_train, y_val = train_test_split(dataset.x, dataset.y, train_size=0.8, test_size=0.2, shuffle=True)

    
    # reduce total dataset size
