...
from model import AgilePredictor
...
model_name = f"agl_bs_{batch_size}_sn_{str(sn_gamma)}_lr_{lr}_ep{str(n_epochs)}_seed_{seed}"
...
def train():
    ...
    model = AgilePredictor(output_dim=3).to(device)
