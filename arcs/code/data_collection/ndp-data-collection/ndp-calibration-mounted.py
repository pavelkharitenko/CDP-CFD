...
uav_2 = UAV("sufferer", controller, "uav_2_r1_joint_motor", "imu2", uav_2_ext_z_force_sensors, uav_2_jft_sensors)
...
while curr_sim_time < sim_max_duration:
    ...
    if FLY_CIRCULAR:
        ...
        if curr_sim_time >= 7.0 and np.round(curr_sim_time,3) % 5.0 == 0.0:
            uav_1_dest = plan_next_coords(1.2, 0.7, uav_1.states[-1][:3], uav_2_dest)
            print("##### Setting new UAV 1 dest", uav_1_dest)

        px4_input_1 = waypoint_after([0.0,4,12],[(0,1,1),(0,0,1), uav_1_dest])
        #px4_input_1 = (0.0, nan, nan, nan, nan, nan, nan, nan, nan, nan, 0.0, nan) # disable uav 1
    ...
    mounted_jft_sensor_list.append(uav_2_mounted_jft_sensor[2])
    print("Recorded forces on UAV 2 mount:", uav_2_mounted_jft_sensor[2])
...
plot_uav_statistics([uav_1, uav_2], mounted_jft_meas=mounted_jft_sensor_list)
