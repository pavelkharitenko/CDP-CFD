"""
Position Controller based on nonlinear feedback from "Neural-Fly enables rapid learning for agile flight in strong winds",
O'Connell et al.
"""
...
class NonlinearFeedbackController:
    ...
    def __init__(self, uav_mass=3.035, target_yaw=0.0, dt=0.005):
        ...
        self.k_p = np.array([4.0, 4.0, 12.0])
        self.k_i = np.array([0.5, 0.0, 0.0]) 
    ...
    def pc_nf(self, desired):
        ...
        u = (
            self.uav_mass * q_r_dot_dot + self.G - self.k_p * s
        )  # - self.k_i * self.s_int
