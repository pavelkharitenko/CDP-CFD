...
class Planner:
    ...
    def _smooth_acceleration_profile(self, total_time, num_waypoints):
        ...
        # Maintain constant velocity after acceleration
        constant_velocity_points = num_waypoints - 2 * acceleration_points
        acceleration = np.concatenate((
            acceleration,  # Acceleration phase
            np.zeros(constant_velocity_points),  # Constant velocity phase
            -acceleration[::-1]  # Deceleration phase
        ))
        return acceleration
    ...
    def _generate_linear_trajectory(self):
        ...
        acceleration_distance = 0.5 * self.velocity * self.acceleration_time  # Distance = 0.5 * v * t

        # If the total distance is less than the distance required for acceleration and deceleration,
        # adjust the acceleration time to fit the distance

        # Distance covered at constant velocity
        velocity_distance = distance + 2 * acceleration_distance

        # Time spent at constant velocity
        velocity_time = velocity_distance / self.velocity

        # Total time = acceleration time + constant velocity time + deceleration time
        total_time = velocity_time

        # Number of waypoints
        num_waypoints = int(total_time / self.dt) + 1

        # Generate a smooth acceleration profile
        acceleration_points = int(self.acceleration_time / self.dt)
        acceleration = np.linspace(0, self.velocity / self.acceleration_time, acceleration_points)
        deceleration = -acceleration[::-1]  # Reverse for deceleration

        # Combine acceleration, constant velocity, and deceleration phases
        accelerations = np.concatenate((
            acceleration,  # Acceleration phase
            np.zeros(num_waypoints + 2 * acceleration_points),  # Constant velocity phase
            deceleration  # Deceleration phase
        ))
...
#planner = Planner(start=(0.0, -2.5, 0.0), end=(0.0, 7.0, 0.0), dt=0.1, velocity=1.0, acceleration_time=1.0)
